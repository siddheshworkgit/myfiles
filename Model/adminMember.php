...
class Member
{
    ...
    private $applicationUrl = 'http://localhost/loginsystem/';
    ...
    {
        $query = 'SELECT * FROM adminconsole where username = ?';
        $paramType = 's';
        $paramValue = array(
            $username
        );
    ...
    $mail->Username = 'gpmpasshelper@gmail.com'; // YOUR gmail email
    $mail->Password = 'lheu nbiu ldya fwjo'; // YOUR gmail password

    // Sender and recipient settings
    $mail->setFrom('gpmpasshelper@gmail.com', 'Reset Password');
    $mail->addAddress ($memberListAry[0]["email"]);
    $mail->addReplyTo('gpmpasshelper@gmail.com', 'Reset Password'); // to set the reply to

    // Setting the email content
    $resetUrl = '<a href="' . $this->applicationUrl . 'reset-password-admin.php?token=' . $token . '">reset</a>';
    $mail->IsHTML(true);
    $mail->Subject = 'Reset password';
    $mail->Body = 'Hi, </br>To reset your password, click this link ' . $resetUrl;
    $mail->AltBody = 'Plain text message body for non-HTML email client. Gmail SMTP email body.';

    $mail->send();
echo '<script>alert("Reset Link Send To Your Registered Email Address")</script>';
} catch (Exception $e) {
    ...
    public function updatePassword($id, $password)
    {
        ...
        $query = 'UPDATE adminconsole SET password = ? WHERE id = ?';
        $paramType = 'si';
        $paramValue = array(
            $hashedPassword,
            $id
