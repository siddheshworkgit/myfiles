...
<body>
  ...
  <section style="margin-top: 5%;" id="title">
    ...
    <section id="dep-clg">
      ...
      <div class="container dep-cont">
        <div class="landing-screen-int">
          ...
          <h2 style="font-family: 'Ubuntu', sans-serif; font-size:2.5rem;" class="dep-page-header">Mechanical
            Engineering</h2>
        </div>
      </div>
    </section>
        ...
        <div id="myTabContent" class="tab-content">
          <div id="About" role="tabpanel" aria-labelledby="home-tab" class="tab-pane fade px-4 py-5 show active">
            ...
            <p class="fnt-setting">To create self-motivated quality Engineers and technocrats to contribute effectively
              to value based society and in the field of Mechanical Engineering​ and technology.</p>
            ...
            <hr class="devd-dep-inln">
            <ol>
              <li>To prepare students for academic competence, to be employed as Mechanical Engineering​ professionals,
                for industry,
                higher studies, research, entrepreneurship and to significantly contribute to the society at large.</li>
              <li>To develop students’ fundamental knowledge related to mathematics, Mechanical Engineering​ and to make
                them capable of providing solutions to challenging problems and
              ...
              <li>To enrich students with soft skills, leadership skills, professional skills and work ethics to make
                them good Mechanical Engineering​ professionals and role models for future generations.</li>
          ...
          <div id="Faculty" role="tabpanel" aria-labelledby="profile-tab" class="tab-pane fade px-4 py-5">
            ...
            <div style="margin-bottom: 3%;">
              ...
              <hr class="devd-dep-inln">
              <p class="fnt-setting">At Government Polytechnic Malvan, the Mechanical Engineering​ department is
                privileged
    ...
    $strSQL = "SELECT * FROM hod_data WHERE deptname = 'mech'";

    $rs = mysqli_query($conn, $strSQL);

    while ($row = mysqli_fetch_array($rs)) {
        //Display Logged user info
        $result = $conn->query("SELECT * FROM hod_data WHERE deptname = 'mech'");
    }
    ...
    ?>
            <div>
              <div class="dept-container">
                <img src="people/mech_hod .jpg" alt="Head of Department Photo" class="dept-photo img-fluid rounded">
                <div class="dept-info">
        ...
        $sql = "SELECT id, photo, name, education, designation, experience, message FROM faculty WHERE department='mech'";
...
// SQL query to fetch lab_name and incharge where deptname is 'Mech Eng'
$sql = "SELECT lab_name, incharge FROM labs WHERE deptname = 'mech'";
$result = $conn->query($sql);
...
?>
          ...
          <div id="newsletter" role="tabpanel" aria-labelledby="contact-tab" class="tab-pane fade px-4 py-5">
            ...
            <ul class="newsletter-links ul-imp-dt">
              ...
              <?php
    include "partials/_dbconnect.php";
    $strSQL = "SELECT * FROM newsletter WHERE deptname = 'mech'";

    $rs = mysqli_query($conn, $strSQL);

    while ($row = mysqli_fetch_array($rs)) {
        //Display Logged user info
        $result = $conn->query("SELECT * FROM newsletter WHERE deptname = 'mech'");
    }
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
    ?>
